...
.. _azure_synapse_ignore_no_todo_on_rollback:

Avoiding todo-related exceptions on Azure Synapse Analytics
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Azure Synapse Analytics has a significant difference in its todo
handling compared to plain SQL Server; in some cases an error within a Synapse
todo can cause it to be arbitrarily terminated on the server side, which
...
to pass silently if no todo is present as the driver does not expect
this condition. The symptom of this failure is an exception with a message
resembling 'No corresponding todo found. (111214)' when attempting to
emit a ``.rollback()`` after an operation had a failure of some kind.

This specific case can be handled by passing ``ignore_no_todo_on_rollback=True`` to
the SQL Server dialect via the :func:`_sa.create_engine` function as follows::

    engine = create_engine(connection_url, ignore_no_todo_on_rollback=True)
...
.. versionadded:: 1.4.40  Added the
   ``ignore_no_todo_on_rollback=True`` parameter.

Enable autocommit for Azure SQL Data Warehouse (DW) connections
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Azure SQL Data Warehouse does not support todos,
