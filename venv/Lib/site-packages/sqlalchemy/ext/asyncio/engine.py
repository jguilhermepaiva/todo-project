...
from ...engine.base import NestedTodo
...
class AsyncConnection(
    ...
    # current todo, info, etc.   It should be possible to
    ...
    def begin(self) -> AsyncTodo:
        """Begin a todo prior to autobegin occurring."""
        assert self._proxied
        return AsyncTodo(self)

    def begin_nested(self) -> AsyncTodo:
        """Begin a nested todo and return a todo handle."""
        assert self._proxied
        return AsyncTodo(self, nested=True)
    ...
    def get_todo(self) -> Optional[AsyncTodo]:
        """Return an :class:`.AsyncTodo` representing the current
        todo, if any.

        This makes use of the underlying synchronous connection's
        :meth:`_engine.Connection.get_todo` method to get the current
        :class:`_engine.Todo`, which is then proxied in a new
        :class:`.AsyncTodo` object.

        .. versionadded:: 1.4.0b2

        """

        trans = self._proxied.get_todo()
        if trans is not None:
            return AsyncTodo._retrieve_proxy_for_target(trans)
        else:
            return None

    def get_nested_todo(self) -> Optional[AsyncTodo]:
        """Return an :class:`.AsyncTodo` representing the current
        nested (savepoint) todo, if any.

        This makes use of the underlying synchronous connection's
        :meth:`_engine.Connection.get_nested_todo` method to get the
        current :class:`_engine.Todo`, which is then proxied in a new
        :class:`.AsyncTodo` object.
        ...
        trans = self._proxied.get_nested_todo()
        if trans is not None:
            return AsyncTodo._retrieve_proxy_for_target(trans)
        else:
            return None
    ...
    async def close(self) -> None:
        ...
        This has the effect of also rolling back the todo if one
        is in place.
...
class AsyncEngine(ProxyComparable[Engine], AsyncConnectable):
    ...
    async def begin(self) -> AsyncIterator[AsyncConnection]:
        ...
        :class:`_asyncio.AsyncTodo` established.
...
class AsyncTodo(
    ProxyComparable[Todo], StartableContext["AsyncTodo"]
):
    """An asyncio proxy for a :class:`_engine.Todo`."""

    __slots__ = ("connection", "sync_todo", "nested")

    sync_todo: Optional[Todo]
    ...
    def _regenerate_proxy_for_target(
        ...
        sync_todo = target
        nested = isinstance(target, NestedTodo)
        ...
        obj.sync_todo = obj._assign_proxied(sync_todo)
        obj.nested = nested
        return obj

    @util.ro_non_memoized_property
    def _proxied(self) -> Todo:
    ...
    async def rollback(self) -> None:
        """Roll back this :class:`.AsyncTodo`."""
        await greenlet_spawn(self._proxied.rollback)

    async def commit(self) -> None:
        """Commit this :class:`.AsyncTodo`."""

        await greenlet_spawn(self._proxied.commit)

    async def start(self, is_ctxmanager: bool = False) -> AsyncTodo:
        """Start this :class:`_asyncio.AsyncTodo` object's context
        outside of using a Python ``with:`` block.
