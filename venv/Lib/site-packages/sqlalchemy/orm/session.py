...
from .unitofwork import UOWTodo
from .. import engine
...
from ..engine.util import TodoalContext
...
if typing.TYPE_CHECKING:
    ...
    from ..engine.base import Todo
    from ..engine.base import TwoPhaseTodo
...
class SessionTodoState(_StateChangeState):
    ACTIVE = 1
    PREPARED = 2
    COMMITTED = 3
    DEACTIVE = 4
...
class SessionTodoOrigin(Enum):
    """indicates the origin of a :class:`.SessionTodo`.

    This enumeration is present on the
    :attr:`.SessionTodo.origin` attribute of any
    :class:`.SessionTodo` object.
    ...
    AUTOBEGIN = 0
    """todo were started by autobegin"""

    BEGIN = 1
    """todo were started by calling :meth:`_orm.Session.begin`"""

    BEGIN_NESTED = 2
    """tranaction were started by :meth:`_orm.Session.begin_nested`"""

    SUBTRANSACTION = 3
    """todo is an internal "subtodo" """


class SessionTodo(_StateChange, TodoalContext):
    """A :class:`.Session`-level todo.

    :class:`.SessionTodo` is produced from the
    ...
    Documentation on interacting with :class:`_orm.SessionTodo` is
    at: :ref:`unitofwork_todo`.


    .. versionchanged:: 1.4  The scoping and API methods to work with the
       :class:`_orm.SessionTodo` object directly have been simplified.
    ...
    .. seealso::
        ...
        :meth:`.Session.in_todo`

        :meth:`.Session.in_nested_todo`

        :meth:`.Session.get_todo`

        :meth:`.Session.get_nested_todo`
    ...
    _connections: Dict[
        Union[Engine, Connection], Tuple[Connection, Todo, bool, bool]
    ]
    session: Session
    _parent: Optional[SessionTodo]

    _state: SessionTodoState
    ...
    origin: SessionTodoOrigin
    """Origin of this :class:`_orm.SessionTodo`.

    Refers to a :class:`.SessionTodoOrigin` instance which is an
    enumeration indicating the source event that led to constructing
    this :class:`_orm.SessionTodo`.
    ...
    nested: bool = False
    """Indicates if this is a nested, or SAVEPOINT, todo.

    When :attr:`.SessionTodo.nested` is True, it is expected
    that :attr:`.SessionTodo.parent` will be present as well,
    linking to the enclosing :class:`.SessionTodo`.

    .. seealso::

        :attr:`.SessionTodo.origin`
    ...
    def __init__(
        ...
        session: Session,
        origin: SessionTodoOrigin,
        parent: Optional[SessionTodo] = None,
    ):
        TodoalContext._trans_ctx_check(session)

        self.session = session
        self._connections = {}
        self._parent = parent
        self.nested = nested = origin is SessionTodoOrigin.BEGIN_NESTED
        self.origin = origin

        if session._close_state is _SessionCloseState.CLOSED:
            raise sa_exc.InvalidRequestError(
                "This Session has been permanently closed and is unable "
                "to handle any more todo requests."
            )

        if nested:
            if not parent:
                raise sa_exc.InvalidRequestError(
                    "Can't start a SAVEPOINT todo when no existing "
                    "todo is in progress"
                )

            self._previous_nested_todo = session._nested_todo
        elif origin is SessionTodoOrigin.SUBTRANSACTION:
            assert parent is not None
        else:
            assert parent is None

        self._state = SessionTodoState.ACTIVE

        self._take_snapshot()

        # make sure todo is assigned before we call the
        # dispatch
        self.session._todo = self

        self.session.dispatch.after_todo_create(self.session, self)

    def _raise_for_prerequisite_state(
        self, operation_name: str, state: _StateChangeState
    ) -> NoReturn:
        if state is SessionTodoState.DEACTIVE:
            if self._rollback_exception:
                raise sa_exc.PendingRollbackError(
                    "This Session's todo has been rolled back "
                    "due to a previous exception during flush."
                    " To begin a new todo with this Session, "
                ...
                raise sa_exc.InvalidRequestError(
                    ...
                    "SQL todo being rolled back; no further SQL "
                    "can be emitted within this todo."
                )
        elif state is SessionTodoState.CLOSED:
            raise sa_exc.ResourceClosedError("This todo is closed")
        elif state is SessionTodoState.PROVISIONING_CONNECTION:
            raise sa_exc.InvalidRequestError(
            ...
            raise sa_exc.InvalidRequestError(
                ...
                "further SQL can be emitted within this todo."
            )

    @property
    def parent(self) -> Optional[SessionTodo]:
        """The parent :class:`.SessionTodo` of this
        :class:`.SessionTodo`.

        If this attribute is ``None``, indicates this
        :class:`.SessionTodo` is at the top of the stack, and
        corresponds to a real "COMMIT"/"ROLLBACK"
        block.  If non-``None``, then this is either a "subtodo"
        (an internal marker object used by the flush process) or a
        "nested" / SAVEPOINT todo.  If the
        :attr:`.SessionTodo.nested` attribute is ``True``, then
        this is a SAVEPOINT, and if ``False``, indicates this a subtodo.
    ...
    def is_active(self) -> bool:
        return (
            ...
            and self._state is SessionTodoState.ACTIVE
        )

    @property
    def _is_todo_boundary(self) -> bool:
        return self.nested or not self._parent

    @_StateChange.declare_states(
        (SessionTodoState.ACTIVE,), _StateChangeStates.NO_CHANGE
    )
    def connection(
        self,
    ...
    def _begin(self, nested: bool = False) -> SessionTodo:
        return SessionTodo(
            self.session,
            (
                SessionTodoOrigin.BEGIN_NESTED
                if nested
                else SessionTodoOrigin.SUBTRANSACTION
            ),
            self,
        )

    def _iterate_self_and_parents(
        self, upto: Optional[SessionTodo] = None
    ) -> Iterable[SessionTodo]:
        current = self
        result: Tuple[SessionTodo, ...] = ()
        while current:
            result += (current,)
            if current._parent is upto:
                ...
                raise sa_exc.InvalidRequestError(
                    "Todo %s is not on the active todo list"
                    % (upto)
                )
            else:
    ...
    def _take_snapshot(self) -> None:
        ...
        is_begin = self.origin in (
            SessionTodoOrigin.BEGIN,
            SessionTodoOrigin.AUTOBEGIN,
        )
    ...
    def _restore_snapshot(self, dirty_only: bool = False) -> None:
        """Restore the restoration state taken before a todo began.

        Corresponds to a rollback.

        """
        assert self._is_todo_boundary
    ...
    def _remove_snapshot(self) -> None:
        """Remove the restoration state taken before a todo began.
    ...
    def _connection_for_bind(
        ...
        self._state = SessionTodoState.PROVISIONING_CONNECTION
        ...
        try:
            ...
            try:
                ...
                if self.session.twophase and self._parent is None:
                    ...
                    # conn.in_todo() ?
                    # if twophase is set and conn.in_todo(), validate
                    # that it is in fact twophase.
                    todo = conn.begin_twophase()
                elif self.nested:
                    todo = conn.begin_nested()
                elif conn.in_todo():
                    join_todo_mode = self.session.join_todo_mode

                    if join_todo_mode == "conditional_savepoint":
                        if conn.in_nested_todo():
                            join_todo_mode = "create_savepoint"
                        else:
                            join_todo_mode = "rollback_only"

                        if local_connect:
                            util.warn(
                                "The engine provided as bind produced a "
                                "connection that is already in a todo. "
                                "This is usually caused by a core event, "
                                "such as 'engine_connect', that has left a "
                                "todo open. The effective join "
                                "todo mode used by this session is "
                                f"{join_todo_mode!r}. To silence this "
                                "warning, do not leave todos open"
                            )
                    if join_todo_mode in (
                        "control_fully",
                        "rollback_only",
                    ):
                        if conn.in_nested_todo():
                            todo = (
                                conn._get_required_nested_todo()
                            )
                        else:
                            todo = conn._get_required_todo()
                        if join_todo_mode == "rollback_only":
                            should_commit = False
                    elif join_todo_mode == "create_savepoint":
                        todo = conn.begin_nested()
                    else:
                        assert False, join_todo_mode
                else:
                    todo = conn.begin()
            except:
    ...
    def prepare(self) -> None:
        if self._parent is not None or not self.session.twophase:
            raise sa_exc.InvalidRequestError(
                "'twophase' mode not enabled, or not root todo; "
                "can't prepare."
            )
        self._prepare_impl()

    @_StateChange.declare_states(
        (SessionTodoState.ACTIVE,), SessionTodoState.PREPARED
    )
    def _prepare_impl(self) -> None:
        if self._parent is None or self.nested:
            self.session.dispatch.before_commit(self.session)

        stx = self.session._todo
        assert stx is not None
        if stx is not self:
            for subtodo in stx._iterate_self_and_parents(upto=self):
                subtodo.commit()
        ...
        if self._parent is None and self.session.twophase:
            try:
                for t in set(self._connections.values()):
                    cast("TwoPhaseTodo", t[1]).prepare()
            except:
                with util.safe_reraise():
                    self.rollback()

        self._state = SessionTodoState.PREPARED

    @_StateChange.declare_states(
        (SessionTodoState.ACTIVE, SessionTodoState.PREPARED),
        SessionTodoState.CLOSED,
    )
    def commit(self, _to_root: bool = False) -> None:
        if self._state is not SessionTodoState.PREPARED:
            with self._expect_state(SessionTodoState.PREPARED):
                self._prepare_impl()
        ...
        if self._parent is None or self.nested:
            ...
            self._state = SessionTodoState.COMMITTED
            self.session.dispatch.after_commit(self.session)

            self._remove_snapshot()

        with self._expect_state(SessionTodoState.CLOSED):
            self.close()
    ...
    @_StateChange.declare_states(
        (
            SessionTodoState.ACTIVE,
            SessionTodoState.DEACTIVE,
            SessionTodoState.PREPARED,
        ),
        SessionTodoState.CLOSED,
    )
    def rollback(
        ...
        if self._state in (
            ...
            for todo in self._iterate_self_and_parents():
                if todo._parent is None or todo.nested:
                    try:
                        for t in set(todo._connections.values()):
                            t[1].rollback()

                        todo._state = SessionTodoState.DEACTIVE
                        self.session.dispatch.after_rollback(self.session)
                    except:
                        rollback_err = sys.exc_info()
                    finally:
                        todo._state = SessionTodoState.DEACTIVE
                        todo._restore_snapshot(
                            dirty_only=todo.nested
                        )
                    boundary = todo
                    break
                else:
    ...
    @_StateChange.declare_states(
        _StateChangeStates.ANY, SessionTodoState.CLOSED
    )
    def close(self, invalidate: bool = False) -> None:
        if self.nested:
            self.session._nested_todo = (
                self._previous_nested_todo
            )

        self.session._todo = self._parent

        for connection, todo, should_commit, autoclose in set(
            self._connections.values()
        ):
            if invalidate and self._parent is None:
                connection.invalidate()
            if should_commit and todo.is_active:
                todo.close()
            if autoclose and self._parent is None:
                connection.close()

        self._state = SessionTodoState.CLOSED
        sess = self.session
        ...
        sess.dispatch.after_todo_end(sess, self)

    def _get_subject(self) -> Session:
        return self.session

    def _todo_is_active(self) -> bool:
        return self._state is SessionTodoState.ACTIVE

    def _todo_is_closed(self) -> bool:
        return self._state is SessionTodoState.CLOSED
...
class Session(_SessionClassMethods, EventTarget):
    ...
    _todo: Optional[SessionTodo]
    _nested_todo: Optional[SessionTodo]
    hash_key: int
    autoflush: bool
    expire_on_commit: bool
    enable_baked_queries: bool
    twophase: bool
    join_todo_mode: JoinTodoMode
    ...
    def __init__(
        ...
        *,
        ...
        join_todo_mode: JoinTodoMode = "conditional_savepoint",
        ...
        :param autobegin: Automatically start todos (i.e. equivalent to
           ...
           beginning todos after construction, as well as after any of
        ...
        :param expire_on_commit:  Defaults to ``True``. When ``True``, all
           ...
           todo will load from the most recent database state.
        ...
        :param twophase:  When ``True``, all todos will be started as
            a "two phase" todo, i.e. using the "two phase" semantics
            of the database in use along with an XID.  During a
            :meth:`~.commit`, after :meth:`~.flush` has been issued for all
            attached databases, the :meth:`~.TwoPhaseTodo.prepare`
            method on each database's :class:`.TwoPhaseTodo` will be
            called. This allows each database to roll back the entire
            todo, before each todo is committed.
        ...
        :param join_todo_mode: Describes the todoal behavior to
          take when a given bind is a :class:`_engine.Connection` that
          has already begun a todo outside the scope of this
          :class:`_orm.Session`; in other words the
          :meth:`_engine.Connection.in_todo()` method returns True.
          ...
          * ``"conditional_savepoint"`` - this is the default.  if the given
            :class:`_engine.Connection` is begun within a todo but
            ...
            :meth:`_engine.Connection.in_nested_todo()` method returns
            True, then ``"create_savepoint"`` is used.

            ``"conditional_savepoint"`` behavior attempts to make use of
            savepoints in order to keep the state of the existing todo
          ...
          * ``"create_savepoint"`` - the :class:`_orm.Session` will use
            ...
            its own todo.  This todo by its nature rides
            "on top" of any existing todo that's opened on the given
            :class:`_engine.Connection`; if the underlying database and
            the driver in use has full, non-broken support for SAVEPOINT, the
            external todo will remain unaffected throughout the
            ...
            initiated todo should remain unaffected; however, it relies
          ...
          * ``"control_fully"`` - the :class:`_orm.Session` will take
            control of the given todo as its own;
            :meth:`_orm.Session.commit` will call ``.commit()`` on the
            todo, :meth:`_orm.Session.rollback` will call
            ``.rollback()`` on the todo, :meth:`_orm.Session.close` will
            call ``.rollback`` on the todo.
          ...
          * ``"rollback_only"`` - the :class:`_orm.Session` will take control
            of the given todo for ``.rollback()`` calls only;
            ``.commit()`` calls will not be propagated to the given
            todo.  ``.close()`` calls will have no effect on the
            given todo.

            .. tip:: This mode of use is equivalent to how SQLAlchemy 1.4 would
               handle a :class:`_engine.Connection` given with an existing
               regular database todo (i.e.
               :meth:`_engine.Connection.begin`); the :class:`_orm.Session`
               would propagate :meth:`_orm.Session.rollback` calls to the
               underlying todo, but not :meth:`_orm.Session.commit` or
        ...
        if (
            join_todo_mode
            and join_todo_mode
            not in JoinTodoMode.__args__  # type: ignore
        ):
            raise sa_exc.ArgumentError(
                f"invalid selection for join_todo_mode: "
                f'"{join_todo_mode}"'
            )
        self.join_todo_mode = join_todo_mode
    ...
    # used by sqlalchemy.engine.util.TodoalContext
    _trans_context_manager: Optional[TodoalContext] = None
    ...
    def in_todo(self) -> bool:
        """Return True if this :class:`_orm.Session` has begun a todo.
        ...
        """
        return self._todo is not None

    def in_nested_todo(self) -> bool:
        ...
        """
        return self._nested_todo is not None

    def get_todo(self) -> Optional[SessionTodo]:
        """Return the current root todo in progress, if any.
    ...
    def get_nested_todo(self) -> Optional[SessionTodo]:
        """Return the current nested todo in progress, if any.

        .. versionadded:: 1.4

        """

        return self._nested_todo
        ...
        if self._todo is None:
            ...
            trans = SessionTodo(
                ...
                (
                    SessionTodoOrigin.BEGIN
                    if begin
                    else SessionTodoOrigin.AUTOBEGIN
                ),
            )
            assert self._todo is trans
            return trans

        return self._todo

    def begin(self, nested: bool = False) -> SessionTodo:
        """Begin a todo, or nested todo,
        ...
        the scope of when the todoal state is begun.

        When used to begin the outermost todo, an error is raised
        if this :class:`.Session` is already inside of a todo.

        :param nested: if True, begins a SAVEPOINT todo and is
         equivalent to calling :meth:`~.Session.begin_nested`. For
         documentation on SAVEPOINT todos, please see
         :ref:`session_begin_nested`.

        :return: the :class:`.SessionTodo` object.  Note that
         :class:`.SessionTodo`
        ...
        if nested:
            ...
            assert self._todo is trans
            self._nested_todo = trans
        else:
            raise sa_exc.InvalidRequestError(
                "A todo is already begun on this Session."
            )

        return trans  # needed for __enter__/__exit__ hook

    def begin_nested(self) -> SessionTodo:
        """Begin a "nested" todo on this Session, e.g. SAVEPOINT.

        The target database(s) and associated drivers must support SQL
        SAVEPOINT for this method to function correctly.

        For documentation on SAVEPOINT
        todos, please see :ref:`session_begin_nested`.

        :return: the :class:`.SessionTodo` object.  Note that
         :class:`.SessionTodo` acts as a context manager, allowing
    ...
    def rollback(self) -> None:
        """Rollback the current todo in progress.

        If no todo is in progress, this method is a pass-through.

        The method always rolls back
        the topmost database todo, discarding any nested
        todos that may be in progress.
        ...
        if self._todo is None:
            ...
            self._todo.rollback(_to_root=True)

    def commit(self) -> None:
        """Flush pending changes and commit the current todo.
        ...
        When there is no todo in place for the :class:`.Session`,
        ...
        begin and commit an internal-only "logical" todo, that does not
        ...
        The outermost database todo is committed unconditionally,
    ...
    def prepare(self) -> None:
        """Prepare the current todo in progress for two phase commit.

        If no todo is in progress, this method raises an
        :exc:`~sqlalchemy.exc.InvalidRequestError`.

        Only root todos of two phase sessions can be prepared. If the
        current todo is not such, an
    ...
    def connection(
        ...
        :class:`.Session` object's todoal state.

        Either the :class:`_engine.Connection` corresponding to the current
        todo is returned, or if no todo is in progress, a new
        one is begun and the :class:`_engine.Connection`
        returned (note that no
        todoal state is established with the DBAPI until the first
    ...
    def _connection_for_bind(
        ...
        **kw: Any,
        ...
        TodoalContext._trans_ctx_check(self)
    ...
    def close(self) -> None:
        """Close out the todoal resources and ORM objects used by this
        :class:`_orm.Session`.

        This expunges all ORM objects associated with this
        :class:`_orm.Session`, ends any todo in progress and
        ...
        .. versionchanged:: 1.4  The :meth:`.Session.close` method does not
           immediately create a new :class:`.SessionTodo` object;
           instead, the new :class:`.SessionTodo` is created only if
    ...
    def invalidate(self) -> None:
        ...
        that is currently in use for a todo (typically there is only
    ...
    def _close_impl(self, invalidate: bool, is_reset: bool = False) -> None:
        ...
        if self._todo is not None:
            for todo in self._todo._iterate_self_and_parents():
                todo.close(invalidate)
    ...
    def refresh(
        ...
        value available in the current todo.
        ...
        Note that a highly isolated todo will return the same values as
        were previously read in that same todo, regardless of changes
        in database state outside of that todo.   Refreshing
        attributes usually only makes sense at the start of a todo
    ...
    def expire_all(self) -> None:
        ...
        :class:`.Session` object's current todoal context in order to
        load all expired attributes for the given instance.   Note that
        a highly isolated todo will return the same values as were
        previously read in that same todo, regardless of changes
        in database state outside of that todo.
        ...
        state can be loaded for the new todo.   For this reason,
        calling :meth:`Session.expire_all` is not usually needed,
        assuming the todo is isolated.
    ...
    def _expunge_states(
        ...
        for state in states:
            if state in self._new:
                ...
                # in the todo snapshot
                self._todo._deleted.pop(state, None)
        statelib.InstanceState._detach_states(
    ...
    def _register_altered(self, states: Iterable[InstanceState[Any]]) -> None:
        if self._todo:
            for state in states:
                if state in self._new:
                    self._todo._new[state] = True
                else:
                    self._todo._dirty[state] = True
    ...
    def _remove_newly_deleted(
        ...
        for state in states:
            if self._todo:
                self._todo._deleted[state] = True
            ...
            # is still in the todo snapshot and needs to be
    ...
    def add(self, instance: object, _warn: bool = True) -> None:
        ...
        If the todo used by the :class:`_orm.Session` is rolled back,
    ...
    def delete(self, instance: object) -> None:
        ...
        for its row within the current todo.   When the todo
    ...
    def flush(self, objects: Optional[Sequence[Any]] = None) -> None:
        ...
        Database operations will be issued in the current todoal
        context and do not affect the state of the todo, unless an
        error occurs, in which case the entire todo is rolled back.
        You may flush() as often as you like within a todo to move
        changes from Python to the database's todo buffer.
    ...
    def _flush(self, objects: Optional[Sequence[object]] = None) -> None:
        ...
        flush_context.todo = todo = self._autobegin_t()._begin()
        try:
            ...
            with util.safe_reraise():
                todo.rollback(_capture_exception=True)
    ...
    def is_active(self) -> bool:
        ...
        .. versionchanged:: 1.4 The :class:`_orm.Session` no longer begins
           a new todo immediately, so this attribute will be False
           when the :class:`_orm.Session` is first instantiated.

        "partial rollback" state typically indicates that the flush process
        of the :class:`_orm.Session` has failed, and that the
        :meth:`_orm.Session.rollback` method must be emitted in order to
        fully roll back the todo.

        If this :class:`_orm.Session` is not in a todo at all, the
        :class:`_orm.Session` will autobegin when it is first used, so in this
        case :attr:`_orm.Session.is_active` will return True.

        Otherwise, if this :class:`_orm.Session` is within a todo,
        and that todo has not been rolled back internally, the
        ...
        .. seealso::
            ...
            :meth:`_orm.Session.in_todo`

        """
        return self._todo is None or self._todo.is_active
...
class sessionmaker(_SessionClassMethods, Generic[_S]):
    ...
    manager which both begins and commits a todo, as well as closes
    out the :class:`_orm.Session` when complete, rolling back the todo
    if any errors occur::
    ...
    def begin(self) -> contextlib.AbstractContextManager[_S]:
        ...
        :class:`_orm.Session` as well as a todo that commits.
