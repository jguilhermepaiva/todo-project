...
class _TConsSubject(Protocol):
    _trans_context_manager: Optional[TodoalContext]


class TodoalContext:
    """Apply Python context manager behavior to todo objects.

    Performs validation to ensure the subject of the todo is not
    used if the todo were ended prematurely.
    ...
    def _rollback_can_be_called(self) -> bool:
        ...
        It also does not mean that there's a todo in progress, as
        it is usually safe to call rollback() even if no todo is
        present.
    ...
    def _trans_ctx_check(cls, subject: _TConsSubject) -> None:
        ...
        if trans_context:
            if not trans_context._todo_is_active():
                raise exc.InvalidRequestError(
                    "Can't operate on closed todo inside context "
    ...
    def __enter__(self) -> Self:
        ...
        # none for outer todo, may be non-None for nested
    ...
    def __exit__(self, type_: Any, value: Any, traceback: Any) -> None:
        ...
        if type_ is None and self._todo_is_active():
            try:
                self.commit()
            except:
            ...
            try:
                if not self._todo_is_active():
                    if not self._todo_is_closed():
                        self.close()
                else:
                    if self._rollback_can_be_called():
                        self.rollback()
