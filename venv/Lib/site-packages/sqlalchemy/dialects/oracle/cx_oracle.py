...
Two Phase Todos Not Supported (use oracledb)
---------------------------------------------------

Two phase todos are **not supported** under cx_Oracle due to poor driver
support.   The newer :ref:`oracledb` dialect however **does** support two phase
todos and should be preferred.
...
class OracleDialect_cx_oracle(OracleDialect):
    ...
    def get_isolation_level(self, dbapi_connection):
        ...
        # general idea of todo id, have to start one, etc.
        # https://stackoverflow.com/questions/10711204/how-to-check-isoloation-level

        # how to decode xid cols from v$todo to match
        ...
        with dbapi_connection.cursor() as cursor:
            # this is the only way to ensure a todo is started without
            # actually running DML.   There's no way to see the configured
            # isolation level without getting it from v$todo which
            # means todo has to be started.
            outval = cursor.var(str)
            cursor.execute(
                """
                begin
                   :trans_id := dbms_todo.local_todo_id( TRUE );
                end;
                """,
                {"trans_id": outval},
            )
