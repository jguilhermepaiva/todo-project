...
def save_obj(base_mapper, states, uowtodo, single=False):
    """Issue ``INSERT`` and/or ``UPDATE`` statements for a list
    of objects.

    This is called within the context of a UOWTodo during a
    ...
    if not single and not base_mapper.batch:
        for state in _sort_states(base_mapper, states):
            save_obj(base_mapper, [state], uowtodo, single=True)
        return
    ...
    for (
        ...
        row_switch,
        update_version_id,
    ) in _organize_states_for_save(base_mapper, states, uowtodo):
    ...
    for table, mapper in base_mapper._sorted_tables.items():
        ...
        update = _collect_update_commands(
            uowtodo, table, states_to_update
        )
...
def post_update(base_mapper, states, uowtodo, post_update_cols):
    ...
    states_to_update = list(
        _organize_states_for_post_update(base_mapper, states, uowtodo)
    )
    ...
    for table, mapper in base_mapper._sorted_tables.items():
        ...
        update = _collect_post_update_commands(
            base_mapper, uowtodo, table, update, post_update_cols
        )
...
def delete_obj(base_mapper, states, uowtodo):
    ...
    states_to_delete = list(
        _organize_states_for_delete(base_mapper, states, uowtodo)
    )
    ...
    for table in reversed(list(table_to_mapper.keys())):
        ...
        delete = _collect_delete_commands(
            base_mapper, uowtodo, table, states_to_delete
        )
...
def _organize_states_for_save(base_mapper, states, uowtodo):
    ...
    for state, dict_, mapper, connection in _connections_for_states(
        ...
        if (
            not has_identity
            and instance_key in uowtodo.session.identity_map
        ):
            instance = uowtodo.session.identity_map[instance_key]
            existing = attributes.instance_state(instance)

            if not uowtodo.was_already_deleted(existing):
                if not uowtodo.is_deleted(existing):
                    util.warn(
                    ...
                    uowtodo.remove_state_actions(existing)
                    row_switch = existing
...
def _organize_states_for_post_update(base_mapper, states, uowtodo):
    ...
    """
    return _connections_for_states(base_mapper, uowtodo, states)


def _organize_states_for_delete(base_mapper, states, uowtodo):
...
def _collect_post_update_commands(
    base_mapper, uowtodo, table, states_to_update, post_update_cols
):
    """Identify sets of values to use in UPDATE statements for a
...
def _emit_post_update_statements(
    base_mapper, uowtodo, mapper, table, update
):
    """Emit UPDATE statements corresponding to value lists collected
...
def _emit_delete_statements(
    base_mapper, uowtodo, mapper, table, delete
):
...
def _finalize_insert_update_commands(base_mapper, uowtodo, states):
...
def _postfetch_post_update(
    mapper, uowtodo, table, state, dict_, result, params
):
    needs_version_id = (
        mapper.version_id_col is not None
        and mapper.version_id_col in mapper._cols_by_table[table]
    ...
    if not uowtodo.is_deleted(state):
...
def _connections_for_states(base_mapper, uowtodo, states):
    ...
    # to use for update
    if uowtodo.session.connection_callable:
        connection_callable = uowtodo.session.connection_callable
    else:
        connection = uowtodo.todo.connection(base_mapper)
