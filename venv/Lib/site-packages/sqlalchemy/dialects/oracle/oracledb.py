...
.. seealso::
    ...
    as well, with the exception that oracledb supports two phase todos.
...
Two Phase Todos Supported
--------------------------------

Two phase todos are fully supported under oracledb. Starting with
oracledb 2.3 two phase todos are supported also in thin mode.    APIs
for two phase todos are provided at the Core level via
:meth:`_engine.Connection.begin_twophase` and :paramref:`_orm.Session.twophase`
for transparent ORM use.

.. versionchanged:: 2.0.32 added support for two phase todos
