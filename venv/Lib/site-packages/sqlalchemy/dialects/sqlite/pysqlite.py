...
Serializable isolation / Savepoints / Todoal DDL
-------------------------------------------------------

In the section :ref:`sqlite_concurrency`, we refer to the pysqlite
driver's assortment of issues that prevent several features of SQLite
from working correctly.  The pysqlite DBAPI driver has several
long-standing bugs which impact the correctness of its todoal
behavior.   In its default mode of operation, SQLite features such as
SERIALIZABLE isolation, todoal DDL, and SAVEPOINT support are
non-functional, and in order to use these features, workarounds must
be taken.

The issue is essentially that the driver attempts to second-guess the user's
intent, failing to start todos and sometimes ending them prematurely, in
...
The good news is that with a few events, we can implement todoal
...
Above, we intercept a new pysqlite connection and disable any todoal
integration.   Then, at the point at which SQLAlchemy knows that todo
scope is to begin, we emit ``"BEGIN"`` ourselves.

When we take control of ``"BEGIN"``, we can also control directly SQLite's
locking modes, introduced at
`BEGIN TRANSACTION <https://sqlite.org/lang_todo.html>`_,
...
.. seealso::
    ...
    `sqlite3 SELECT does not BEGIN a todo <https://bugs.python.org/issue9924>`_ -
    on the Python bug tracker

    `sqlite3 module breaks todos and potentially corrupts data <https://bugs.python.org/issue10740>`_ -
    on the Python bug tracker
