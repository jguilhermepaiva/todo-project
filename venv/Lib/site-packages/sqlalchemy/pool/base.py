...
class PoolResetState:
    ...
    __slots__ = ("todo_was_reset", "terminate_only", "asyncio_safe")

    todo_was_reset: bool
    """Indicates if the todo on the DBAPI connection was already
    essentially "reset" back by the :class:`.Connection` object.

    This boolean is True if the :class:`.Connection` had todoal
    state present upon it, which was then not closed using the
    :meth:`.Connection.rollback` or :meth:`.Connection.commit` method;
    instead, the todo was closed inline within the
...
class Pool(log.Identified, event.EventTarget):
    ...
    def __init__(
         ...
         * ``"rollback"`` - call rollback() on the connection,
           to release locks and todo resources.
         ...
         * ``"commit"`` - call commit() on the connection,
           ...
           any data changes present on the todo
...
class _ConnectionRecord(ConnectionPoolEntry):
    ...
    def checkout(cls, pool: Pool) -> _ConnectionFairy:
        ...
        rec.fairy_ref = ref = weakref.ref(
            ...
            lambda ref: (
                _finalize_fairy(
                    None, rec, pool, ref, echo, todo_was_reset=False
                )
...
class _ConnectionFairy(PoolProxiedConnection):
    ...
    def _checkin(self, todo_was_reset: bool = False) -> None:
        _finalize_fairy(
            self.dbapi_connection,
    ...
    def _reset(
        ...
        if pool._reset_on_return is reset_rollback:
            if todo_was_reset:
                if self._echo:
                    pool.logger.debug(
                        "Connection %s reset, todo already reset",
                        self.dbapi_connection,
                    )
            else:
                if self._echo:
    ...
    def _close_special(self, todo_reset: bool = False) -> None:
        self._counter -= 1
        if self._counter == 0:
            self._checkin(todo_was_reset=todo_reset)
