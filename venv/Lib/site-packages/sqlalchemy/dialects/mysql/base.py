...
For fully atomic todos as well as support for foreign key
constraints, all participating ``CREATE TABLE`` statements must specify a
todoal engine, which in the vast majority of cases is ``InnoDB``.
...
All MySQL / MariaDB dialects support setting of todo isolation level both via a
...
class MySQLDialect(default.DefaultDialect):
    ...
    def get_isolation_level(self, dbapi_connection):
        ...
        if self._is_mysql and self.server_version_info >= (5, 7, 20):
            cursor.execute("SELECT @@todo_isolation")
        else:
            cursor.execute("SELECT @@tx_isolation")
        row = cursor.fetchone()
        if row is None:
            util.warn(
                "Could not retrieve todo isolation level for MySQL "
                "connection."
            )
            raise NotImplementedError()
        val = row[0]
