...
Most SQLAlchemy dialects support setting of todo isolation level
...
characteristics of the todo, which is in addition to the isolation level
...
todo, it does not cover a wider range of session-level state, including
...
will end todos in progress as well as discard temporary tables
...
will remain consistent with the state of the todo::
...
it is invoked outside of the scope of any todo and therefore will not
...
be invoked outside of a todo block.   The Python DBAPI enforces that
even for a single statement, a todo is present, so to use this
...
class PGDialect(default.DefaultDialect):
    ...
    def get_isolation_level(self, dbapi_connection):
        cursor = dbapi_connection.cursor()
        cursor.execute("show todo isolation level")
    ...
    def do_rollback_twophase(
        ...
        if is_prepared:
            if recover:
                # FIXME: ugly hack to get out of todo
                # context when committing recoverable todos
