...
.. note:: The implementation for the
   ...
   Oracle dialect necessarily forces the start of a todo using the
   ...
   raise an exception if the ``v$todo`` view is not available due to
   ...
   exception due to ``v$todo`` not being readable as well as any other
...
.. versionchanged:: 1.3.22 In the event that the default isolation
   level cannot be read due to permissions on the v$todo view as
