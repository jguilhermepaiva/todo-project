...
.. warning::  The ``asyncpg`` database driver necessarily uses caches for
   ...
   stale, nor can it retry the statement as the PostgreSQL todo is
    ...
    async def _prepare_and_execute(self, operation, parameters):
        ...
        async with adapt_connection._execute_mutex:
            if not adapt_connection._started:
                await adapt_connection._start_todo()
...
class AsyncAdapt_asyncpg_connection(AdaptedConnection):
    ...
    async def _async_ping(self):
        if self._todo is None and self.isolation_level != "autocommit":
            # create a tranasction explicitly to support pgbouncer
            # todo mode.   See #10226
            tr = self._connection.todo()
            await tr.start()
            try:
    ...
    async def _start_todo(self):
        if self.isolation_level == "autocommit":
            return

        try:
            self._todo = self._connection.todo(
                isolation=self.isolation_level,
                readonly=self.readonly,
                deferrable=self.deferrable,
            )
            await self._todo.start()
        except Exception as error:
            self._handle_exception(error)
        else:
            self._started = True
    ...
    def rollback(self):
        if self._started:
            try:
                self.await_(self._todo.rollback())
            except Exception as error:
                self._handle_exception(error)
            finally:
                self._todo = None
