...
class AsyncSession(ReversibleProxy[Session]):
    ...
    def get_todo(self) -> Optional[AsyncSessionTodo]:
        """Return the current root todo in progress, if any.

        :return: an :class:`_asyncio.AsyncSessionTodo` object, or
         ``None``.

        .. versionadded:: 1.4.18

        """
        trans = self.sync_session.get_todo()
        if trans is not None:
            return AsyncSessionTodo._retrieve_proxy_for_target(trans)
        else:
            return None

    def get_nested_todo(self) -> Optional[AsyncSessionTodo]:
        ...
        trans = self.sync_session.get_nested_todo()
        if trans is not None:
            return AsyncSessionTodo._retrieve_proxy_for_target(trans)
        else:
            return None
    ...
    async def connection(
        ...
        **kw: Any,
        ...
        this :class:`.Session` object's todoal state.

        This method may also be used to establish execution options for the
        database connection used by the current todo.
    ...
    def begin(self) -> AsyncSessionTodo:
        """Return an :class:`_asyncio.AsyncSessionTodo` object.

        The underlying :class:`_orm.Session` will perform the
        "begin" action when the :class:`_asyncio.AsyncSessionTodo`
        object is entered::
        ...
        todo is begun, as database todos begin on an
        on-demand basis.  However, the begin block is async to accommodate
        for a :meth:`_orm.SessionEvents.after_todo_create`
        ...
        return AsyncSessionTodo(self)

    def begin_nested(self) -> AsyncSessionTodo:
        """Return an :class:`_asyncio.AsyncSessionTodo` object
        which will begin a "nested" todo, e.g. SAVEPOINT.
        ...
        return AsyncSessionTodo(self, nested=True)
    ...
    async def commit(self) -> None:
        """Commit the current todo in progress.
    ...
    def in_todo(self) -> bool:
        r"""Return True if this :class:`_orm.Session` has begun a todo.
    ...
    def in_nested_todo(self) -> bool:
        ...
        return self._proxied.in_nested_todo()
...
class async_sessionmaker(Generic[_AS]):
    ...
    def begin(self) -> _AsyncSessionContextManager[_AS]:
        ...
        :class:`_orm.AsyncSession` as well as a todo that commits.
...
class AsyncSessionTodo(
    ReversibleProxy[SessionTodo],
    StartableContext["AsyncSessionTodo"],
):
    """A wrapper for the ORM :class:`_orm.SessionTodo` object.

    This object is provided so that a todo-holding object
    for the :meth:`_asyncio.AsyncSession.begin` may be returned.

    The object supports both explicit calls to
    :meth:`_asyncio.AsyncSessionTodo.commit` and
    :meth:`_asyncio.AsyncSessionTodo.rollback`, as well as use as an
    ...
    __slots__ = ("session", "sync_todo", "nested")

    session: AsyncSession
    sync_todo: Optional[SessionTodo]
    ...
    def is_active(self) -> bool:
        return (
            self._sync_todo() is not None
            and self._sync_todo().is_active
        )

    def _sync_todo(self) -> SessionTodo:
        if not self.sync_todo:
            self._raise_for_not_started()
        return self.sync_todo

    async def rollback(self) -> None:
        """Roll back this :class:`_asyncio.AsyncTodo`."""
        await greenlet_spawn(self._sync_todo().rollback)

    async def commit(self) -> None:
        """Commit this :class:`_asyncio.AsyncTodo`."""

        await greenlet_spawn(self._sync_todo().commit)

    async def start(
        self, is_ctxmanager: bool = False
    ) -> AsyncSessionTodo:
        self.sync_todo = self._assign_proxied(
            await greenlet_spawn(
        ...
        if is_ctxmanager:
            self.sync_todo.__enter__()
        return self

    async def __aexit__(self, type_: Any, value: Any, traceback: Any) -> None:
        await greenlet_spawn(
            self._sync_todo().__exit__, type_, value, traceback
        )
